...
CardAssetInfo.getRewardIcon = function(item){
    if( !CardAssetInfo.iconRect )CardAssetInfo.iconRect = CardAssetInfo.getRewardIconPositions();
...
CardRewadItem.prototype.getIcon = function(){
    var tx = new Bitmap( new BitmapData( Assets.assets().textureTripleSceneCropped, this.rect ) );
    tx.x = this.position.x;
    tx.y = this.position.y;
    return tx;
}
